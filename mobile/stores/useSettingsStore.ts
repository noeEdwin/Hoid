...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  setDailyReviewLimit: (limit: number) => {
    ...
    writeSettingsFile({ dailyReviewLimit: clamped, deckReviewHistory });
  },

  markDeckReviewed: (deckId: string) => {
    const today = getTodayString();
    const { dailyReviewLimit, deckReviewHistory } = get();
    const updated = { ...deckReviewHistory, [deckId]: today };
    set({ deckReviewHistory: updated });
    writeSettingsFile({ dailyReviewLimit, deckReviewHistory: updated });
  },
  ...
  resetAllDeckReviews: () => {
    ...
    set({ deckReviewHistory: {} });
    writeSettingsFile({ dailyReviewLimit, deckReviewHistory: {} });
  },
